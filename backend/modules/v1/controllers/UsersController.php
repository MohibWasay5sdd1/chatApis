...
class UsersController extends ActiveController
{
    ...
    public function actionLogin()
    {
        ...
        if ($model) {
            ...
            if ($is_password_correct) {
                if ($model['status']== 'Unverified') {
                    ...
                    if ($model_token) {
                        ...
                        if($userdata['profile_pic_url'] == null){
                            $profile_pic = 'null';
                        } else {
            ...
            $model=$user->getUserByUsername($user_email);
            if ($model) {
                //echo json_encode()
                $password_db=$model['user_password'];
                $is_password_correct = password_verify($password, $password_db);
                ...
                if ($is_password_correct) {
                    if ($model['status']== 'Unverified') {
                        ...
                        if ($model_token) {
                            $userdata = $user->getUserByUsername($user_email);
    ...
    public function actionInvitation($id)
    {
        ...
        if ($model) {
            ...
            if($invite){
                ...
                echo json_encode(array(
                    'status'=>200,
                    'data'=>array('message'=>"Invitation sent.")),JSON_PRETTY_PRINT
                );
            } else {
                Yii::$app->response->statusCode=400;
                echo json_encode(array(
